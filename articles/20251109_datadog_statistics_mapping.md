...
| **順序尺度** | 順序関係のみ意味を持つ | HTTPステータスコード(*2)、ログレベル(DEBUG < INFO < WARN < ERROR) | ヒストグラム |
| **名義尺度** | カテゴリや名前 | 環境名(prod/staging)、サービス名、ホスト名 | カウント |

*1: 温度や西暦など代表例で、Observabilityには関係が薄いので割愛。
*2: 厳密に順序尺度と言えるかは微妙ですが、ステータスを数値順に意味付けしているので、ここでは順序尺度に分類してます
...
| APM | HTTPスタータス | 順序尺度 | `ステータスコード>= 500` でアラート |
